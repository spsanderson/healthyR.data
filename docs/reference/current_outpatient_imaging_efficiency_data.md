...
- .data_sets:
  ...
  only the Imaging Efficiency data sets are available to the function to
  choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Imaging Efficiency data from the current hospital data
file.

## Details

This function will obtain the current Imaging Efficiency data from the
output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

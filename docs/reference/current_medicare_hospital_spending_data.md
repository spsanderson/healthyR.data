...
- .data_sets:
  ...
  bring back all of the data in the Medicare Hospital Spending data sets
  that are in the file. You can choose from the following:
  ...
  only the Medicare Hospital Spending data sets are available to the
  function to choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Medicare Hospital Spending data from the current
hospital data file.

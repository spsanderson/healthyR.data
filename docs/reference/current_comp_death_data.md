...
- .data_sets:
  ...
  back all of the data sets that are in the Complications and death data
  sets. You can choose from the following:
  ...
  only the Complications and Death data sets are available to the
  function to choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Complications and Death data from the current hospital
data file.

## Details

This function will obtain the current Complications and Death data from
the output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

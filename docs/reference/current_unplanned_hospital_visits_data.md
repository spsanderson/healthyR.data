...
``` r
current_unplanned_hospital_vists_data(
  .data,
...
- .data_sets:
  ...
  all of the data in the Unplanned Hospital Visits data sets that are in
  ...
  only the Unplanned Hospital Visits data sets are available to the
  function to choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Unplanned Hospital Visits data from the current
hospital data file.

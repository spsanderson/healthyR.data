...
- .data_sets:
  ...
  The default is: c("Facility","State","National"), which will bring
  back all of the data sets that are in the hospital associated
  infections data sets. You can choose from the following:
  ...
  only the hospital associated infections data sets are available to the
  function to choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current hospital associated infection data from the current
hospital data file.

## Details

This function will obtain the current hospital associated infections
data from the output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

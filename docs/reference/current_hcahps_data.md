...
- .data_sets:
  ...
  back all of the data sets that are in the HCAPS data sets. You can
  ...
  only the hcahps data sets are available to the function to choose from
  and `grep` is used to find matches with `ignore.case = TRUE` set.
...
This function will obtain the current HCAPHS data from the output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

...
current_hvbp_data(
  .data,
  .data_sets = c("Outcomes", "Efficiency", "Engagement", "Safety", "TPS")
)
```
...
- .data_sets:
  ...
  The default is: c("Outcomes","TPS"), which will bring back all of the
  data sets that are in the hospital VBP data sets. You can choose from
  the following:
  ...
  You can also pass things like c("tps","Safety") as behind the scenes
  only the hospital vbp data sets are available to the function to
...
This function will obtain the current hospital VBP data from the output
of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

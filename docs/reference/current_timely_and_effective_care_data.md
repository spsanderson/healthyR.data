...
- .data_sets:
  ...
  all of the data in the Timely & Effective Care data sets that are in
  the file. You can choose from the following:
  ...
  only the Timely & Effective Care data sets are available to the
  function to choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Timely & Effective Care data from the current hospital
data file.

## Details

This function will obtain the current Timely & Effective Care data from
the output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)

...
- .data_sets:
  ...
  back all of the data in the Maternal Health data sets that are in the
  ...
  only the Maternal Health data sets are available to the function to
  choose from and `grep` is used to find matches with
  `ignore.case = TRUE` set.

## Value

Gets the current Maternal Health data from the current hospital data
file.

## Details

This function will obtain the current Maternal Health data from the
output of the
[`current_hosp_data()`](https://www.spsanderson.com/healthyR.data/reference/current_hosp_data.md)
